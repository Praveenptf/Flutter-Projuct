...
class _BookingPageState extends State<BookingPage> {
  ...
  Future<void> _fetchServices() async {
    try {
      final response = await http.get(
        Uri.parse('http://192.168.1.10:8080/parlour/$shopId'),
        headers: {
        ...
        setState(() {
          ...
          if (jsonResponse.isNotEmpty && jsonResponse[0]['items'] != null) {
            ...
            services = List<Map<String, dynamic>>.from(
                jsonResponse[0]['items'].map((service) {
              return {
                ...
                // 'itemImage':
                //     service['itemImage'], // If you want to display images
              };
            }));
          } else {
  ...
  Future<void> _fetchEmployees() async {
    try {
      final response = await http.get(
        Uri.parse(
            'http://192.168.1.10:8080/employees/by-parlourId?parlourId=$shopId'),
        headers: {
          'Content-Type': 'application/json',
          'Cookie':
  ...
  Future<void> _bookAppointment() async {
    ...
    String? userId = await TokenManager.getUser_Id(); // Retrieve the user ID
    ...
    final response = await http.post(
      Uri.parse('http://192.168.1.10:8080/api/cart/add'),
      headers: {
        'Content-Type': 'application/json',
