  ...
  Future<void> _handleRegister() async {
    if (_formKey.currentState!.validate()) {
      final response = await http.post(
        Uri.parse('http://192.168.1.8:8080/user/UserReg'),
        headers: {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        String userId =
            responseData['userId']; // Assuming the user ID is in the response

        print('Token: $token');
        print('User  ID: $userId');

        // Store the token and user ID using TokenManager
        await TokenManager.storeToken(token);
        await TokenManager.storeUser_Id(userId); // Store the user ID
